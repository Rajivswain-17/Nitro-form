...
app.post("/api/v2/forms/:slug/submit", async (req, res) => {
  ...
  const rawFields = form.fieldsJson as Array<{ id: string; type: string; label: string; required?: boolean; options?: string[] }>;
  const fields = rawFields.map(f => ({ ...f, required: f.required ?? false }));
